...
public class clsData
{
    ...
    /// <summary>
    /// ค้นหาคำใน DataTable (LINQ)
    /// </summary>
    /// <param name="dtSearch">DataTable ที่ใช้ค้นหา</param>
    /// <param name="strWord">คำที่ต้องการค้นหา</param>
    /// <param name="strField">ฟิลด์ที่ต้องการค้นหา กรณีไม่ระบุจะเช็ค Field แรก</param>
    /// <returns>True=พบ False=ไม่พบ</returns>
    public DataTable Search(DataTable dtSearch, string strWord, string strField)
    {
        ...
        */
        ...
        {
            if (!dt.Rows[i][strField].ToString().Contains(strWord))
            {
                dt.Rows[i].Delete();
            }
        }
